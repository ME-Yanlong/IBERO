...
## 15. 工业材料与加工机制扩展计划索引（2026-09-10，已批准）

新增重点转向弹性卡扣解锁与材料去除。详细执行清单追加在 [《Core-0.1 工业材料与加工机制调研》第 13—17 节](04_Core-0.1_工业材料与加工机制调研.md#13-core-01-工业机制扩展详细执行任务计划已批准)，计划编号 `C01-IND-PLAN-1`；该文档第 18 节记录实际进度。

计划包含 S0—S9 共 60 项主线任务，以及 X-01—X-08 共 8 项需单独确认的条件扩展；列明依赖、文件落点、量化门槛、证据要求和连续工作规则。详细文档是该轮任务状态的唯一登记来源，本节只作入口，不复制一份独立清单。

用户已批准从 S0 连续推进，工作限定在 `feature/core01-industrial-materials`。第 14 节的历史完成记录不因此改变；实际实施和验收结果逐阶段记录，不将批准的方案或待办任务标为已完成，不自动合并 main。
